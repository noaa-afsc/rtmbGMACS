...
#'
#' @title Extract a (character) value from a line of text
#' @description Function to extract a (character) value from a line of text.
#' @param txt - input text string (1-element character vector)
#' @param comment - string used to split text into value and comments (default="#")
#' @return a 1-element character vector
#' @details The 1-line input text is split into a character vector using [stringr::str_split_1()] and
#' the first element of the result is retained. Whitespace preceding and following the retained
#' text is also removed (but "interior" whitespace is retained).
#'
#' @examples
#' # example code
...
#' @export
#'
extractTextValue<-function(txt,comment="#"){
  v = stringr::str_split_1(txt,comment)[1];
  v = stringr::str_trim(v);
  return(v);
}

#'
#' @title Extract a numeric value from a line of text
#' @description Function to extract a numeric value from a line of text.
#' @param txt - input text string (1-element character vector)
#' @param comment - string used to split text into value and comments (default="#")
#' @return a 1-element numeric vector
#' @details The 1-line input text is split into a character vector using [stringr::str_split_1()] and
#' the first element of the result is retained. Whitespace preceding and following the retained
#' text is also removed (but "interior" whitespace is retained). The result is converted to numeric
#' using [as.numeric()].
#'
#' @examples
#' # example code
...
#' @export
#'
extractNumericValue<-function(txt,comment="#"){
  v = stringr::str_split_1(txt,comment)[1];
  v = stringr::str_trim(v);
  v = as.numeric(v);
  return(v);
...
#' @title Remove strings starting with a comment character
#' @description Function to remove strings starting with a comment character.
#' @param strv - character vector to remove comment lines from
#' @param comment - character(s) to regard as indicating a comment
#' @return a character vector with elements that don't start with a comment character
#'
#' @examples
...
#'
#' @importFrom stringr str_starts str_trim
#' @export
#'
